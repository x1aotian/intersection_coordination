function output = general_case_NB_BS_Pic(Ls,Do,Di,Nv,V,Bdy,T,N,NK)
    ...
    %% Cell of path generate functions
    F_cell = cell(Nv,2);
    for i = 1:Nv
    ...
    for K = 1:NK
        ...
        I_col_m{1,K} = I_col_set;
        R_col_m{1,K} = R_col_set;
        IR_col_m{1,K} = IR_col_set;
        ...
        for i = 1:Nv
            xux_all{i,2*K} = w_opt(3*Nv-3+i); %u0
    ...
    %% Some Easy Plot
    plot_set = ['--rx'; '--gx';'--bx'];
    plot_x = zeros(NK+1,Nv);
    for i = 1:Nv
        for j = 1:NK+1
            plot_x(j,i) = pxy_trj(2*i-1,j);
            plot_y(j,i) = pxy_trj(2*i,j);
        end
        plot(plot_x(:,i),plot_y(:,i),plot_set(i,:));
        hold on
    end
end
