function output = general_case_gif_ellip(Ls,Do,Di,Nv,V,Bdy,T,N,NK)
    ...
    for i = 1:Nv
        ...
        [rec_x,rec_y] = rec_gen(pxy_trj(2*i-1,1),pxy_trj(2*i,1),0,car_ls,car_ss);
    ...
    for K = 1:NK
        ...
        for k = 0+K-1:N+K-2
            ...
            for i = 1:I_len
                ...
                a_the = F_cell{a_index,3}(a_p);
                b_the = F_cell{b_index,3}(b_p);
                
                Delta = MX.sym(['Del' num2str(cnt_delta)], 2);
                cnt_delta = cnt_delta + 1;
                w_delta = [w_delta {Delta}];
                
                elcons = ellip_cons(car_ls,car_ss,a_cor,a_the,b_cor,b_the,Delta);
        ...
        final_w = [vertcat(w{:});vertcat(w_delta{:})];
        w_size = size(vertcat(w{:}),1);
        w0 = zeros(size(final_w,1),1);
        prob = struct('f',J,'x',final_w,'g',vertcat(g{:}));
        ...
        for i = 1:Nv
            ...
            [rec_x, rec_y] = rec_gen(pxy_trj(2*i-1,K+1),pxy_trj(2*i,K+1),angle,car_ls,car_ss);
        ...
        if K == 1
            ...
            figcaptext = text(2, 2, sprintf('MPC loop %d', K), 'Interpreter', 'latex');
        else
